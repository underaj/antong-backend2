...
module.exports = class extends Base {
    ...
    async createOrdersAction() {
        ...
        for (let index = 0; index < students.length; index++) {
            ...
            let use_car = this.model("v_orders").where({
        ...
        for (let index = 0; index < students.length; index++) {
            ...
            //添加学生信息
            if (element.coachId == "") {
                stuId = await this.queryStudent(element.name, element.tel, element.carType, data.coachId);
            }
            for (let j = 0; j < element.timecode.length; j++) {
                ...
                //先将车辆锁单
                let id = await this.model("timetable_detail").where({ id: useCar.id }).update({ is_available: 1 });
        ...
        if (flag === 0) {
            ...
            if (!think.isEmpty(carId)) {
                await this.model("timetable_detail").where({ "id": ["in", carId] }).update({ is_available: 0 });
            }
            //清空carId数据信息
            carId = "";
            parameter = "";
    ...
    async queryOrdersByIdAction() {
        ...
        //根据order订单号去查询学生信息
        let stu = await this.model("order_timetable as a").join("student b ON a.`students_id`=b.`student_id` ").join("car c ON a.`car_id`=c.`id` ")
            .field("a.id,a.`orders_id`,a.`time_code`,b.`name`,b.`tel`,c.`car_type`,c.car_no,c.`car_card`, b.student_id, a.print,a.is_print,a.print_num")
            .where({
                "a.orders_id": data.id
            }).select();
        let coachData = await this.model("orders").join("coach b ON orders.coach_id=b.id").where({ "orders.id": stu[0].orders_id }).find();
        //返回查询结果
        data.students = stu;
    ...
    async queryWelfareOrderAction() {
        ...
        ).where({ "v_main.order_type": 3 }).group("v_main.id")
            .order("v_main.id DESC")
            .page(pageIndex, pageSize).countSelect();
        //统计结果数据长度 做分页统计
        let countsize = await query01.where({ "v_main.order_type": 3 }).count("DISTINCT id");
        return this.success({
            "data": data,
            "count": countsize
        });
    ...
    async queryCarTimeAction() {
        ...
        let data = await query.where({ "a.is_available": 0 }).field("a.* ").select();
        return this.success(data);
    }
    ...
    async cancelOrderAction() {
        ...
        //释放车辆信息 
        let carId = await this.model("order_timetable").where({ orders_id: orders.id }).select();
        for (let index = 0; index < carId.length; index++) {
            const element = carId[index];
            await this.model("timetable_detail").where({ id: element.timetable_id }).update({ is_available: 0 });
        }
    ...
    async approvalOrdersAction() {
        ...
        let id = data.id;
        //判断该订单是否存在
        let orders_exits = await this.model("orders").where({ id: ["in", id], order_type: 3 }).select();
        ...
        if (type == 1) {
            ...
            for (let index = 0; index < orders_exits.length; index++) {
                ...
                await this.model("orders").where({ id: element.id }).update({ order_check: 1, pay_type: 1 });
            }
            ...
            for (let index = 0; index < orders_exits.length; index++) {
                ...
                //更改为不通过
                await this.model("orders").where({ id: element.id }).update({ order_check: 2 });
            }
        } else {
            return this.fail(1000, "参数错误");
        }
    ...
    async cancelReturnAmountAction() {
        ...
        for (let index = 0; index < id.length; index++) {
            ...
            let data = await query.
                ...
                .where({ "orders.id": element, "b.print": 1 }).find();
            //如果不为空 则说明有未打单的
            if (think.isEmpty(data)) {
            ...
            if (type == 2) {
                //取消返点
                await this.model("orders").where({ id: element }).update({ is_return: 2 });
            } else {
                // 加时单  后台下单操作
    ...
    async printOdersAction() {
        ...
        console.log(id);
        //判断该条信息是否存在
        let data = await this.model("order_timetable").where({ id: ["in", id] }).select();
        if (think.isEmpty(data)) {
        ...
        if (type == 1) {
            ...
            for (let index = 0; index < id.length; index++) {
                ...
                let is_all_print = await this.model("order_timetable").where({ orders_id: orderId, print: 0 }).select();
                //如果为空 则表示都已完成打单  更新主表的字段收据
                if (think.isEmpty(is_all_print)) {
                    await this.model("orders").where({ id: orderId }).update({
                        collection_type: 2,
                        update_by: "admin",
                        pay_type: 1,
                        update_time: moment(new Date()).format("YYYY-MM-DD HH:mm:ss")
            ...
            //判断是否已补单
            let orders_is_print = await this.model("order_timetable").where({ id: ["in", id] }).find();
            for (let index = 0; index < orders_is_print.length; index++) {
                ...
                //补单
                await this.model("order_timetable").where({ id: element.id }).update({
                    is_print: 1,
                    update_by: "admin",
                    update_time: moment(new Date()).format("YYYY-MM-DD HH:mm:ss")
                });
    ...
    async queryMaxTimeAndMinTimeAction() {
        ...
        let data = await this.model("timetable").join("timetable_detail b ON timetable.id=b.timetable_id ")
    ...
    async createOrdersByXiaoChengxuAction() {
        ...
        if (!think.isEmpty(data.coachId)) {
            ...
            if (is_true) {
                ...
                await this.model("coach").where({ id: data.coachId }).update({ "is_blacklist": 1 });
            }
        }
        let coachInfor = await think.model("coach").where({ id: data.coachId }).find();
        ...
        */
        ...
        //   orders.id = ordersId;
        //发送站内信信息
        let detail = "您预约的考试时间为 " + data.date + "的订单已成功保留车位，请在15分钟内完成支付"
   ...
   */
    async autoReturnAmountAction() {
        ...
        for (let i = 0; i < orders.length; i++) {
            ...
            //查询付款记录拿到商户订单号
            let payment = await this.model("payment").where({ orders_id: element.id, status: "SUCCESS" }).find();
    ...
    async queryStudent(name, tel, carType, coachId) {
        console.log("学生添加开始====")
        let students = await think.model("student").where({ name: name, tel: tel }).find();
        let s_id = null;
        ...
        if (think.isEmpty(isBlacklistUpdateTime)) {
            ...
            if (think.isEmpty(orderListByUpdateTime)) {
                ...
                for (var j = 0; j < orderListByUpdateTime.length; j++) {
                    ...
                    var orderTimetableListByUpdateTime = await this.model('order_timetable').where({ orders_id: ordersIdByUpdateTime, print: 0 }).select();
    ...
    async checkCoachBlackList(coachId) {
        ...
        let countSize = await this.model("orders").join("order_timetable b ON  orders.id=b.orders_id")
            .where({ coach_id: coachId, "orders.order_time": { '<': moment().format("YYYY-MM-DD") }, "b.print": 0, "b.is_black": 0, "STATUS": 1 })
            .select();
    ...
    async autoCancelOrdersByTimeCronAction() {
        ...
        let data = await this.model("orders").where({ status: 1, pay_type: 2 }).select();
        if (think.isEmpty(data)) {
        ...
        for (let index = 0; index < data.length; index++) {
            ...
            var createTime = moment(element.create_time).add(15, "minute").format("YYYY-MM-DD HH:mm:ss"); // 15分钟
            ...
            if (moment(newDate).isAfter(createTime)) {
                let orders = await this.model("orders").where({ id: element.id }).find();
                if (think.isEmpty(orders)) {
                    continue;
                }
                //查询是否付款，付款需要先退款，没有付款则取消其它数据信息 释放车辆信息
                ...
                for (let index = 0; index < carId.length; index++) {
                    ...
                    await this.model("timetable_detail").where({ id: ss.timetable_id }).update({ is_available: 0 });
                }
                //发送站内信 通知订单已取消
                let detail = "您预约的考试时间为 " + element.order_time + "的订单未在15分钟内完成支付,已取消该订单"
                await this.model("user_in_mail").add({
                    detail: detail,
                    create_time: moment(new Date()).format("YYYY-MM-DD HH:mm:ss"),

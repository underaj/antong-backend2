...
module.exports = class extends Base {
    ...
    async queryCarTimeTableAction() {
        ...
        let data = await querySql.where({ "b.type": type, "car.is_delete": 1 }).page(pageIndex, pageSize).countSelect();//.page(pageIndex, pageSize).countSelect();
    ...
    async addCarTimeTableAction() {
        console.info("调用添加车辆排期方法。。。。。。。。。" + JSON.stringify(this.post()));
        //获取排班日期 是一个数组对象
        ...
        for (let i = 0; i < carId.length; i++) {
            ...
            //根据车辆id查询车号
            let car_no = await this.model("car").where({ id: car_Id }).field("car_no").find();
            //拿到日期数组做循环遍历
            for (let j = 0; j < date.length; j++) {
                const date01 = date[j];
                //根据车辆id 日期查询是否已经存在该日期的数据信息
                let is_ture = await this.model("timetable").where({ "date": date01, "car_id": car_Id, "type": type }).find();
                let id = null;
                ...
                let use_car = await query_car_timetable.where({ date: date01, car_id: car_Id, type: type == 1 ? 2 : 1 }).find();
                ...
                for (let index = 0; index < detail.length; index++) {
                    ...
                    //用于做判断是否已经排过相同的时间排期了
                    let aaa_ = await this.model("timetable").join("timetable_detail   ON timetable.id=timetable_detail.timetable_id").where({ "timetable.id": id, "timetable_detail.date_logo": element.datelogo, "timetable_detail.timecode": element.timecode, "timetable_detail.is_delete": 0 })
                        .find();
                    ...
                    if (!think.isEmpty(use_car)) {
                        ...
                        if (is_exit == 1) {
                            //跟时间值取相应的时间段
                            let infomation = await time_code.where({
        ...
        if (think.isEmpty(return_msg)) {
            ...
            for (let i = 0; i < return_msg.length; i++) {
                ...
                //上下午
                let date_logo = element.date_logo == 1 ? "上午" : element.date_logo == 2 ? "下午" : "晚上";
                //时间段
                let timecode = this.convertutil(element.timecode, type);
                console.log("打印时间段信息----》" + timecode)
                //拼接字符串
                mm = mm + element.date + ",车号:" + element.car_no + "与科目" + type + date_logo + "时间段("
                    + timecode + ")排期存在冲突;";
            }
    ...
    async selectCarTimeTableByCarNo() {
        console.info("调用车辆已排期(selectCarTimeTableByCarNo)查询方法。。。。。。。。。");
        //车类型
        ...
        //组装sql语句
        let query = this.model("car as a").join("timetable as b on a.`id`=b.`car_id` ");
        if (!think.isEmpty(carNo)) {
            query.where({ "a.car_id": carNo });
        }
        //查询出当前车的排期时间数据
        let data = await query.where({ "b.type": project, "a.car_type": cartype }).field("a.`id`,a.`car_type`,b.`date`,b.`is_available`,b.`time_code_id`").select();
        return this.success(data);
    }
    /**
    ...
    async deleteCarTimeTableAction() {
        ...
        //删除明细表数据信息
        await this.model("timetable_detail").where({ timetable_id: data.id }).delete();
        //删除主排班表数据信息
    ...
    async updateCarTimeAction() {
        ...
        let is_use = await this.model("timetable a ")
            ...
            .where({ "a.id": timedate.id, "b.is_available": 1 }).select();
        if (is_use.length > 0) {
        ...
        const addTimetable = this.model("timetable");
        //添加信息数据
        let id = await addTimetable.update(cartime = {
            date: timedate.date,
        ...
        for (let i = 0; i < timedate.detail.length; i++) {
            ...
            //循环执行添加
            await this.model("timetable_detail").add(cardetail = {
                timetable_id: id,
    ...
    async selectCarAction() {
        ...
        let data = await querycar.where({ is_delete: 1 }).field("id,car_no,car_card,car_type").select();
        return this.success(data);
    }
    ...
    async demoCarAction() {
        ...
        for (let index = 0; index < tt.length; index++) {
            ...
            let car_detail = await this.model("timetable").
                ...
                where({ "timetable.car_id": element.id, "timetable_detail.is_delete": 0, "timetable.type": type }).
                field("DISTINCT timetable.date,timetable.id,GROUP_CONCAT(timetable_detail.id)AS str")
                .group("timetable.id")
                .select();
            ...
            for (let j = 0; j < car_detail.length; j++) {
                ...
                //查询出明细数据封装
                let _carDetail = await car_detail_query.where({ "id": ["in", _detail.str] }).select();
                let date_car = {
                    "date": _detail.date,
                    "list": _carDetail
                };
    ...
    async updateCarTimeTableAction() {
        ...
        if (!think.isEmpty(updateList)) {
            ...
            for (let i = 0; i < updateList.length; i++) {
                ...
                //查询是否有过预订数据
                let car_time = await this.model("order_timetable").where({ car_id: carId, timetable_id: element }).select();
            ...
            //直接修改为已开放的状态
            await this.model("timetable_detail").where({ id: ["in", updateStr] }).update({ is_delete: 1 });
        ...
        if (!think.isEmpty(insertList)) {
            ...
            var is_str = [];
            //根据是时间  类型 车号 
            let timetableId = await this.model("timetable").where({ date: date, car_id: carId, type: type }).find();
            if (!think.isEmpty(timetableId)) {
                ...
                for (let index = 0; index < insertList.length; index++) {
                    ...
                    let is_exite = await this.model("timetable").join("timetable_detail  b ON timetable.id=b.timetable_id").
                        where({ "timetable.date": date, "timetable.car_id": carId, "timetable.type": type, "b.timecode": element.timecode, "b.date_logo": element.date_logo })
                        .find();
                    //查询是否重复
                    let use_car = await query_car_timetable.where({ date: date, car_id: carId, type: type == 1 ? 2 : 1 }).find();
                    if (!think.isEmpty(use_car)) {
                        //只存在上下午中会有时间重叠的现象
                ...
                if (!think.isEmpty(is_str)) {
                    await this.model("timetable_detail").where({ id: is_str }).update({ is_delete: 0 });
                }
            }
    ...
    async deleteCarTimetableAction() {
        ...
        let type = this.post("type");
        //查询出该日期下所有的排班信息
        let allTimetable = await this.model("timetable").where({ date: date, type: type }).field("id").select();
        if (!think.isEmpty(allTimetable)) {
            ...
            await this.model("timetable_detail").where({ "timetable_id": ["in", codeArray] }).delete();
    ...
    async queryTimeCarAction() {
        ...
        let allTimetable = await this.model("timetable")
            .join("timetable_detail b ON timetable.id=b.timetable_id")
            .where({ date: date, type: type, "b.is_available": 1 }).field("DISTINCT car_id").select();
        let codeArray = [];
        if (!think.isEmpty(allTimetable)) {
            for (let index = 0; index < allTimetable.length; index++) {
                const element = allTimetable[index];
